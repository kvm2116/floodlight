...
public class ForwardingGroupsAssignmentResource extends ServerResource {
	 ...
	 *        {
	 *        	"u'00:65:5c:8a:38:3e:cd:28": {"0": 29, "1": 29}, 
	 *        	"u'00:65:2c:23:3a:3e:ed:a9": {"0": 53, "1": 53}
	 *        }
	...
	public static Map<String, Object> jsonToStorageEntry(String fmJson) throws IOException {
		...
		try {
			...
			for(String s : JSONObject.getNames(jo)){
				...
				String dpidStr = s.toString().replaceAll("'", "");
				String dpid = dpidStr.substring(1, dpidStr.length()-1);
				// create a hashMap with new group assignments for this dpid
				...
				Forwarding.groupAssignments.put(dpid, mapping);
			}
	 ...
	 * @throws IOException If there was an error parsing the JSON
	 */
	public static Map<String, Object> jsonToStorageEntryIP(String fmJson) throws IOException {
		String new_string = fmJson.replace("\\", "");
		String json_string = new_string.substring(1, new_string.length() - 1);
		JSONObject jo = null;
		try {
